...
def train(inputs, model, criterion, optimizer):
    train_X, train_y, train_seq_len = inputs
    epoch_loss = 0
    correct = 0
    total = 0
    last_total = 0
    ...
    num_batch = int(math.ceil(train_X.size(0)/float(batch_size)))
    ...
    with click.progressbar(range(num_batch)) as batch_indexes:
        for batch_i in batch_indexes:
            ...
            if (batch_i+1)*batch_size > train_X.size(0):
...
def validate(inputs, model, criterion):

    with torch.no_grad():

        val_X, val_y, val_seq_len = inputs
        epoch_loss = 0
        correct = 0
        total = 0
        last_total = 0
        ...
        batch_size = model.batch_size
        num_batch = int(math.ceil(val_X.size(0)/float(batch_size)))
        
        with click.progressbar(range(num_batch)) as batch_indexes:
            for batch_i in batch_indexes:
                end_ind = (batch_i+1)*batch_size
                if (batch_i+1)*batch_size > val_X.size(0):
                    end_ind = val_X.size(0)
                start_ind = end_ind - batch_size

                mb_X = val_X[start_ind:end_ind]
                mb_len = val_seq_len[start_ind:end_ind]

                all_mask, last_mask = sequence_masks(sequence_length=mb_len, max_len=val_X.size(1))
                all_mask = all_mask.flatten().float()
                last_mask = last_mask.flatten().float()

                mb_y = val_y[start_ind:end_ind]
                mb_y = mb_y.view(-1, 1).repeat(1, val_X.size(1)).flatten()
                outputs = model(mb_X, mb_len)
                loss = criterion(outputs, mb_y)
...
def run(loader, model, criterion, optimizer, early_stopping, early_stopping_interval, checkpoint_file, num_epochs, restore = True):
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        train_loss, avg_acc, last_acc = train(mb, model, criterion, optimizer)
        ...
        mb = loader.load_next_batch('val')
        val_loss, avg_acc, last_acc = validate(mb, model, criterion)
    ...
    test_loss, avg_acc, last_acc = validate(inputs['test'], model, criterion)
